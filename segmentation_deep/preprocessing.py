...
def resize_mask(fn):
    ...
    image_file.save((fn.parent.parent) / "train_masks-256" / fn.name)
...
def recolor_mask(fn):
    ...
    img.save((fn.parent.parent) / "train_masks_bw-256" / fn.name)


def resize_img(fn):
    PIL.Image.open(fn).resize((256, 256)).save(
        (fn.parent.parent) / "train-256" / fn.name
    )
...
if __name__ == "__main__":
    ...
    files = list((data_path / "train_masks-256").iterdir())
    with concurrent.futures.ThreadPoolExecutor(8) as e:
